...
const MainDashboard: React.FC<MainDashboardProps> = memo(({
    ...
    return (
        <div className="max-w-6xl mx-auto w-full flex flex-col font-sans">
            ...
            {/* Minimal Daily Challenge Strip */}
            <div className="mb-5 px-4 py-2.5 rounded-xl bg-card border border-border shadow-2xs flex items-center justify-between gap-3 text-xs">
                <div className="flex items-center gap-2.5 truncate">
                    <span className="text-sm">🔥</span>
                    <span className="font-bold text-foreground truncate">Daily Workout:</span>
                    <span className="text-muted-foreground truncate hidden sm:inline">Master 30 words every day in 15 minutes</span>
                </div>
                <button
                    onClick={() => navigate('/daily-exercise')}
                    className="shrink-0 bg-primary/10 hover:bg-primary/20 text-primary font-bold px-3 py-1.5 rounded-lg text-xs transition-colors flex items-center gap-1.5 cursor-pointer"
                >
